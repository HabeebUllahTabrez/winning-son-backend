...
// MigrateEntries receives a list of journal entries and upserts them for the authenticated user.
func (h *JournalHandler) MigrateEntries(w http.ResponseWriter, r *http.Request) {
	userID := r.Context().Value("userID").(int)
	...
	response := map[string]interface{}{
		"message":       "Entries migrated successfully",
